...
class TarifFioul(models.Model):
    type_fioul = models.ForeignKey("TypeFioul", on_delete=models.CASCADE)
    zone = models.ForeignKey("Zone", on_delete=models.CASCADE)
...
class TarifLivraison(models.Model):
    type_livraison = models.ForeignKey("TypeLivraison", on_delete=models.CASCADE)
    zone = models.ForeignKey("Zone", on_delete=models.CASCADE)
    ...
    def candidat_valide(self, candidat):
        jours_feries = JourFerie.objects.filter(date__gte=datetime.now()).values_list(
            "date", flat=True
        )
    ...
    def date_livraison(self):
        ...
        if (
            candidat.hour >= config_fioul.heure_limite_livraison
            or not self.candidat_valide(candidat)
        ):
            reste += 1
        index = 0
...
class Zone(models.Model):
    distributeur = models.ForeignKey(
        "Distributeur", null=True, on_delete=models.CASCADE
    )
    nom = models.CharField(max_length=255)
    ...
    def livraisons(self):
        return self.tariflivraison_set.filter(actif=True).order_by(
            "type_livraison__position"
        )
...
class Panier(models.Model):
    ...
    adresse_livraison = models.ForeignKey(
        "Adresse", related_name="+", null=True, blank=True, on_delete=models.CASCADE
    )
    adresse_facturation = models.ForeignKey(
        "Adresse", related_name="+", null=True, blank=True, on_delete=models.CASCADE
    )
    type_fioul = models.ForeignKey(
        "TypeFioul", null=True, blank=True, on_delete=models.CASCADE
    )
    type_livraison = models.ForeignKey(
        "TypeLivraison", null=True, blank=True, on_delete=models.CASCADE
    )
    ...
    def valeur(self):
        ...
        decote = TarifDecote.objects.get(
            zone=self.zone, min__lte=self.qte, max__gte=self.qte
        )
        ...
        for liv in TarifLivraison.objects.filter(zone=self.zone, actif=True):
            ...
            if self.type_livraison != liv.type_livraison:
                liv_prix = (
                    liv.extra * Decimal(self.qte) / Decimal(1000)
                ) - liv_prix_base
                if liv_prix > 0:
                    liv_label = "+ " + localize(floatformat(liv_prix, "-2")) + " euros"
                elif liv_prix < 0:
                    liv_label = (
                        "- " + localize(floatformat(-1 * liv_prix, "-2")) + " euros"
                    )
    ...
    def set_client(self, client):
        self.client = client
        self.adresse_livraison = client.adresse_set.filter(
            nom_adresse="adresse_par_defaut"
        )[0]
...
class Commande(models.Model):
    ...
    def data(self):
        if self.livraison_ttc < 0:
            prix_livraison = "- %s" % localize(
                floatformat(-1 * self.livraison_ttc, "-2")
            )
        else:
        ...
        d = {
            ...
            "qte": self.qte,
            "prix_litre": localize(
                floatformat(self.total_ttc / Decimal(self.qte), "-4")
            ),
...
class Message(models.Model):
    type = models.ForeignKey("TypeMessage", on_delete=models.CASCADE)
    user = models.ForeignKey(User, blank=True, null=True, on_delete=models.CASCADE)
    client = models.ForeignKey(
        "Client", blank=True, null=True, on_delete=models.CASCADE
    )
    commande = models.ForeignKey(
        "Commande", blank=True, null=True, on_delete=models.CASCADE
    )
