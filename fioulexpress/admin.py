...
class CodePostalResource(resources.ModelResource):
    ...
    def import_row(self, row, instance_loader, dry_run=False, **kwargs):
        if dry_run:
            if row.get('zone__distributeur__nom', False) and row.get('zone__nom', False):
                result = super(CodePostalResource, self).import_row(row, instance_loader, dry_run=False, **kwargs)
                instance = CodePostal.objects.get(id=result.object_id)
                if (result.import_type != result.IMPORT_TYPE_NEW) and (not(instance.zone) or instance.zone.nom != row.get('zone__nom')):
            ...
            if row.get('zone__distributeur__nom', False) and row.get('zone__nom', False):
                distributeur, created = Distributeur.objects.get_or_create(nom=row.get('zone__distributeur__nom'))
                if row.get('zone__nom', False):
                    zone, created = Zone.objects.get_or_create(distributeur=distributeur, nom=row.get('zone__nom'))
                    result = super(CodePostalResource, self).import_row(row, instance_loader, dry_run=False, **kwargs)
                    instance = CodePostal.objects.get(id=result.object_id)
    ...
    def import_row(self, row, instance_loader, dry_run=False, **kwargs):
        ...
        if dry_run:
            if row.get('distributeur__nom', False) and row.get('nom', False):
                ...
                if distribs:
                    zones = Zone.objects.filter(nom=row.get('nom'), distributeur__nom=row.get('distributeur__nom'))
                    if zones:
            ...
            if row.get('distributeur__nom', False) and row.get('nom', False):
                ...
                distrib, created = Distributeur.objects.get_or_create(nom=row.get('distributeur__nom'))
                if not distrib.user:
                ...
                tarif_fioul, created = zone.tariffioul_set.get_or_create(type_fioul_id=2)
                tarif_fioul.prix_ttc = Decimal(row.get('prix_ordinaire', False) or 0)
                tarif_fioul.actif = tarif_fioul.prix_ttc != Decimal(0)
                tarif_fioul.save()

                tarif_fioul, created = zone.tariffioul_set.get_or_create(type_fioul_id=3)
                tarif_fioul.prix_ttc = Decimal(row.get('prix_superieur', False) or 0)
                tarif_fioul.actif = tarif_fioul.prix_ttc != Decimal(0)
                tarif_fioul.save()

                tarif_fioul, created = zone.tariffioul_set.get_or_create(type_fioul_id=4)
                tarif_fioul.prix_ttc = Decimal(row.get('prix_gnr', False) or 0)
                tarif_fioul.actif = tarif_fioul.prix_ttc != Decimal(0)
                tarif_fioul.save()

                tarif_fioul, created = zone.tariffioul_set.get_or_create(type_fioul_id=5)
                tarif_fioul.prix_ttc = Decimal(row.get('prix_gnr_superieur', False) or 0)
                tarif_fioul.actif = tarif_fioul.prix_ttc != Decimal(0)
                tarif_fioul.save()

                tarif_fioul, created = zone.tariffioul_set.get_or_create(type_fioul_id=1)
                tarif_fioul.prix_ttc = Decimal(row.get('prix_cristal', False) or 0)
                tarif_fioul.actif = tarif_fioul.prix_ttc != Decimal(0)
                tarif_fioul.save()

                tarif_decote, created = zone.tarifdecote_set.get_or_create(min=500, max=999)
                tarif_decote.decote = Decimal(row.get('decote_1', False) or 0)
                tarif_decote.save()

                tarif_decote, created = zone.tarifdecote_set.get_or_create(min=1000, max=2999)
                tarif_decote.decote = Decimal(row.get('decote_2', False) or 0)
                tarif_decote.save()

                tarif_decote, created = zone.tarifdecote_set.get_or_create(min=3000, max=3999)
                tarif_decote.decote = Decimal(row.get('decote_3', False) or 0)
                tarif_decote.save()

                tarif_decote, created = zone.tarifdecote_set.get_or_create(min=4000, max=10000)
                tarif_decote.decote = Decimal(row.get('decote_4', False) or 0)
                tarif_decote.save()

                tarif_livraison, created = zone.tariflivraison_set.get_or_create(type_livraison_id=2)
                tarif_livraison.actif = True
                tarif_livraison.extra = 0
                tarif_livraison.save()

                tarif_livraison, created = zone.tariflivraison_set.get_or_create(type_livraison_id=1)
                if row.get('livraison_express', False) == 'null':
                    tarif_livraison.extra = Decimal(0)
                    tarif_livraison.actif = False
                else:
                    tarif_livraison.extra = Decimal(row.get('livraison_express', False) or 0)
                    tarif_livraison.actif = True
                tarif_livraison.save()

                tarif_livraison, created = zone.tariflivraison_set.get_or_create(type_livraison_id=3)
                if row.get('livraison_eco', False) == 'null':
                    tarif_livraison.extra = Decimal(0)
                    tarif_livraison.actif = False
                else:
                    tarif_livraison.extra = Decimal(row.get('livraison_eco', False) or 0)
                    tarif_livraison.actif = True
                tarif_livraison.save()
...
class ZoneAdmin(ImportExportModelAdmin):
    ...
    inlines = [TarifFioulInline, TarifDecoteInline, TarifLivraisonInline, ]
...
class CommandeAdmin(admin.ModelAdmin):
    list_display = ['id', 'client_nom', 'type_fioul', 'type_livraison', 'date_livraison', 'statut', 'total_ttc', 'commission_ttc']
