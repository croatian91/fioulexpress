...
def home(request):
    if request.GET.get("msg", False):
        messages.add_message(
            request,
            messages.SUCCESS,
            "<p>Un exemple de validation OK !</p><p>Avec plusieurs petits paragraphes ...</p>",
        )
...
def distrib_hp(request):
    if request.user.is_superuser:
        ...
        zones = (
            Zone.objects.filter(distributeur=request.user.distributeur)
            .order_by("nom")
            .exclude(archive=True)
        )
    else:
    ...
    if (
        ...
        or request.user.groups.filter(name__in=("Comptable", "Webmaster")).exists()
    ):
...
def distrib_commandes(request):
    ...
    if is_admin:
        if request.GET.get("distributeur", False):
            ...
            commandes = Commande.objects.filter(
                zone__distributeur=request.GET.get("distributeur")
            )
        else:
        ...
        commandes = Commande.objects.filter(
            zone__distributeur=request.user.distributeur
        )
    ...
    if request.GET.get("du", False) or request.GET.get("au", False):
        ...
        recherche_form = DistribCommandeForm(
            initial={
                "du": date_du,
                "au": date_au,
            }
    ...
    if request.GET.get("export", False):
        ...
        response[
            "Content-Disposition"
        ] = 'attachment; filename="commandes-%s-%s.csv"' % (
            date_du.strftime("%d%m%Y"),
            date_au.strftime("%d%m%Y"),
        )
        ...
        for c in commandes:
            row = [
                ...
                c.qte,
                c.type_fioul.nom,
                c.type_livraison.nom,
                c.commission_ttc,
                c.total_ttc,
                c.commission_ht,
                c.total_ht,
                c.adresse_livraison.nom,
                c.adresse_livraison.prenom,
                c.adresse_livraison.detail_1,
                c.adresse_livraison.code_postal,
                c.adresse_livraison.detail_4,
            ]
            if c.adresse_facturation:
                row += [
                    c.adresse_facturation.nom,
                    c.adresse_facturation.prenom,
                    c.adresse_facturation.detail_1,
                    c.adresse_facturation.code_postal,
                    c.adresse_facturation.detail_4,
                ]
            else:
                row += [
                    c.adresse_livraison.nom,
...
def distrib_commande_detail(request, id_commande):
    ...
    if not is_admin and commande.zone.distributeur != getattr(
        request.user, "distributeur", False
    ):
        return redirect(reverse("distrib-hp"))
    return render(
        request,
        "distrib/commande_detail.html",
        {
            "admin": is_admin,
            "commande": commande,
            "has_permission": True,
...
def distrib_produits(request):
    ...
    if request.POST:
        ...
        livraisons_formset = DistribTarifLivraisonForm(
            request.POST, instance=zone_active
        )
        if (
            produits_formset.is_valid()
            and decotes_formset.is_valid()
            and livraisons_formset.is_valid()
        ):
            produits_formset.save()
            decotes_formset.save()
            livraisons_formset.save()
            messages.add_message(
                request, messages.SUCCESS, "La zone a été modifiée avec succès"
            )
            return redirect(reverse("distrib-produits") + "?zone=%s" % zone_active.id)
        else:
            messages.add_message(
                request, messages.ERROR, "La zone n'a pas été modifiée"
            )
    else:
    ...
    for tarif_id in (3, 2, 1):
        ...
        if t.actif:
            livraisons.append(t)
    tarifs = []
    for tarif_fioul in zone_active.tariffioul_set.filter(actif=True).order_by(
        "type_fioul__position"
    ):
        ...
        for decote in zone_active.tarifdecote_set.order_by("min"):
            ...
            for livraison in livraisons:
                tarifs_decote_livraison["ligne"].append(
                    round(tarif_fioul.prix_ttc + decote.decote + livraison.extra, 4)
                )
            tarif["decotes"].append(tarifs_decote_livraison)
        tarifs.append(tarif)
    return render(
        request,
        "distrib/produits.html",
        {
            "zones": zones,
            "zone_active": zone_active,
...
def distrib_livraisons(request):
    ...
    if zone_active:
        if request.POST:
            ...
            if zone_form.is_valid():
                ...
                return redirect(
                    reverse("distrib-livraisons") + "?zone=%s" % zone_active.id
                )
            else:
                messages.add_message(
                    request, messages.ERROR, "La zone n'a pas été modifiée"
        ...
        code_postaux = (
            zone_active.codepostal_set.order_by("code_postal")
    ...
    return render(
        request,
        "distrib/livraisons.html",
        {
            "zones": zones,
            "zone_active": zone_active,
            "distributeur_form": distributeur_form,
            # 'livraisons_formset' : livraisons_formset,
            "livraisons": zone_active
            and zone_active.tariflivraison_set.order_by("type_livraison__position")
            or [],
            ...
            or not request.user.distributeur.actif
            or (zone_active and not zone_active.actif),
            "admin": is_admin,
            "has_permission": True,
        },
    )
...
def distrib_remove_cp(request, cp):
    if request.user.is_superuser:
        ...
        zones = Zone.objects.filter(distributeur=request.user.distributeur).order_by(
            "nom"
        )
    if request.GET.get("zone", False):
    ...
    if zone_active:
        if request.POST.get("cp_liste", False):
            ...
            for cp_search in liste_cp.split(","):
                if len(cp_search) > 1:
                    for cp in (
                        CodePostal.objects.filter(code_postal__startswith=cp_search)
            ...
            if not User.objects.filter(username=username):
                if request.POST.get("password") and request.POST.get(
                    ...
                    if password != password_confirm:
                        erreurs[
                            "text"
                        ] = "Le mot de passe est différent de sa confirmation"
                    elif len(password) < 8:
                        erreurs[
                            "text"
                    ...
                    erreurs[
                        "text"
                    ] = "Vous devez renseigner un mot de passe et le confirmer"
            else:
    ...
    if request.POST:
        ...
        if zone_form.is_valid():
            ...
            for type_fioul in TypeFioul.objects.all():
                TarifFioul.objects.create(
                    type_fioul=type_fioul, prix_ttc=10000, zone=zone, actif=False
                )
            for type_livraison in TypeLivraison.objects.all():
                TarifLivraison.objects.create(
                    type_livraison=type_livraison, extra=0, zone=zone, actif=False
                )
...
def client_valide_cp(request):
    try:
        ...
        return redirect("/")
    if (
        not code_postal.zone
        or code_postal.zone.archive
        or not code_postal.zone.actif
...
def prospect_inscrire(request):
    if request.POST.get("email", False):
        Prospect.objects.create(
            email=request.POST.get("email"), code_postal=request.POST.get("cp")
        )
...
def client_commande_valoriser(request):
    ...
    request.session["panier_valeur"]["prix_litre"] = localize(
        floatformat(request.session["panier_valeur"]["prix_litre"], "-3")
    )
    request.session["panier_valeur"]["total_ttc"] = localize(
        floatformat(request.session["panier_valeur"]["total_ttc"], "-2")
    )
    request.session["panier_valeur"]["total_ht"] = localize(
        floatformat(request.session["panier_valeur"]["total_ht"], "-2")
    )

    for k in ["livraison_ttc", "acompte", "acompte_ht", "reste"]:
        request.session["panier_valeur"][k] = localize(
            floatformat(request.session["panier_valeur"][k], -2)
        )
...
def client_commande_livraison(request):
    if request.user and request.user.is_staff:
        messages.error(
            request,
            'Vous etes connecté en tant que membre de l\'équipe, <a href="/distributeur/logout/?next=/">se deconnecter</a>',
        )
        return redirect("/")
    try:
        zone_active = Zone.objects.get(id=request.session["client_zone"])
    ...
    if request.POST:
        ...
        adresse_livraison_form = AdresseForm(
            request.POST, prefix="adresse-livraison", instance=panier.adresse_livraison
        )
        adresse_livraison_form["detail_4"].field.choices = CodePostal.objects.filter(
            code_postal=request.session["client_cp"]
        ).values_list("commune", "commune")
        adresse_facturation_form = AdresseFacturationForm(
            request.POST,
            prefix="adresse-facturation",
            instance=panier.adresse_facturation,
        )
        commentaire_form = PanierForm(
            request.POST, prefix="commentaire", instance=panier
        )
                ...
                if request.POST.get("creer_compte", False):
                    if request.POST.get("creer_compte_password", False):
                        if Client.objects.filter(
                            email=client.email, user__isnull=False
                        ):
                            messages.add_message(
                                request,
                                messages.ERROR,
                                'Un compte existe déjà avec cet email, <a href="/client/connexion/?next=/commande/livraison/">connectez-vous</a>',
                            )
                            return render(
                                request,
                                "commande/livraison.html",
                                {
                                    "zone_active": zone_active,
                                    "panier": panier,
                            ...
                            client.user = user
                            client.save()
                            user_actif = authenticate(
                                username=user.username,
                                password=request.POST.get("creer_compte_password"),
                            )
                            login(request, user_actif)
                    else:
                        return render(
        ...
        adresse_livraison_form = AdresseForm(
            prefix="adresse-livraison", instance=panier.adresse_livraison
        )
        adresse_livraison_form["detail_4"].field.choices = (
            CodePostal.objects.filter(code_postal=request.session["client_cp"])
            .order_by("commune")
            .values_list("commune", "commune")
        )
        ...
        adresse_facturation_form = AdresseFacturationForm(
            prefix="adresse-facturation", instance=instance_panier_adresse_facturation
        )
...
def client_commande_monetico_ok(request):
    try:
        ...
        messages.add_message(
            request, messages.ERROR, "Le paiement n'a pas pu être validé"
        )
    ...
    if request.POST:
        try:
            client = Client.objects.get(
                email=request.POST.get("email"), user__isnull=False
            )
            user_actif = authenticate(
                username=client.user.username, password=request.POST.get("password")
            )
            login(request, user_actif)
            if request.session.get("panier_id", False):
                panier = Panier.objects.get(id=request.session["panier_id"])
...
def client_recuperation_mot_passe(request):
    messages.add_message(
        request,
        messages.INFO,
        "Si votre email correspond à un compte client, vous recevrez dans quelques minutes un email permettant de modifier votre mot de passe.",
    )
    if request.POST.get("email", False):
        ...
        for client in Client.objects.filter(user__isnull=False, email=email):
            ...
            data = {
                ...
                + reverse("client-motdepasse-maj", args=[token])
            }
...
def client_maj_mot_passe(request, token=""):
    try:
        client = Client.objects.get(
            token_password=token, token_date__gte=datetime.now()
        )
    except:
    ...
    if request.POST.get("password", False):
        ...
        client.save()
        messages.add_message(
            request, messages.SUCCESS, "Le mot de passe a été mis à jour"
        )
        return redirect("/")
    return render(
        request,
        "client/maj_mot_de_passe.html",
        {
            "client": client,
        },
    )
...
def admin_commande_pdf(request, commande_id):
    ...
    if (
        getattr(request.user, "distributeur", False)
        and request.user.distributeur != commande.zone.distributeur
    ):
        return redirect("/")
